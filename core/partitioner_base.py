...
class PartitionerBase(ABC):
    ...
    def get_distribution(self, dataset: Dataset, 
        ...
        for client_id, indices in client_indices.items():
            ...
            client_dist = {int(label): int(count) for label, count in zip(unique, counts)}
    ...
    def get_statistics(self, dataset: Dataset,
        ...
        samples_per_client = {client_id: len(indices) 
                             for client_id, indices in client_indices.items()}
        ...
        return {
            ...
            "statistics": {
                ...
                "imbalance_ratio": float(max_samples / min_samples) if min_samples > 0 else float('inf')
            }
        }
