...
# 导入数据库连接
from database.db_connection import DatabaseConfig, get_db
...
class DatasetRegistryImporter:
    ...
    def _prepare_strategy_data(self, strategy: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "default_params" in data and isinstance(data["default_params"], dict):
            data["default_params"] = json.dumps(
        ...
        if "supported_datasets" in data and isinstance(
            data["supported_datasets"], list
        ):
            data["supported_datasets"] = json.dumps(
                data["supported_datasets"], ensure_ascii=False
            )

        return data

    def import_datasets(
        self, json_path: Optional[Path] = None, overwrite: bool = True
    ) -> int:
        """
        导入数据集到数据库
    ...
    def _upsert_dataset(self, data: Dict[str, Any]) -> None:
        ...
        update_clause = ", ".join(
            [f"`{c}` = VALUES(`{c}`)" for c in columns if c != "name"]
        )
    ...
    def export_datasets(
        self, output_path: Optional[Path] = None, include_inactive: bool = False
    ) -> Path:
        """
        从数据库导出数据集到 JSON
...
def register_datasets(json_path: Optional[str] = None, overwrite: bool = True) -> int:
    """
    便捷函数：仅导入数据集
...
def register_strategies(json_path: Optional[str] = None, overwrite: bool = True) -> int:
    """
    便捷函数：仅导入划分策略
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--overwrite", "-o", action="store_true", default=True, help="覆盖已有数据"
    )
    parser.add_argument(
        "--no-overwrite", dest="overwrite", action="store_false", help="不覆盖已有数据"
    )
